defmodule Phauxth.Confirm.PassResetTest do
  ...
  test "reset password succeeds" do
    ...
    {:ok, user} = call_reset("froderick", password, [identifier: :email, key_validity: 60])
    assert user
  end

  test "reset password fails with expired token" do
    password = "C'est bon, la vie"
    {:error, message} = call_reset("froderick", password, [identifier: :email, key_validity: 0])
    assert message =~ "Invalid credentials"
  end

  test "reset password fails when reset_sent_at is nil" do
    {:error, message} = call_reset("igor", "password", [identifier: :email, key_validity: 60])
    assert message =~ "Invalid credentials"
  end
