defmodule Phauxth.ChannelTest do
  ...
  defp call(current_user, opts) do
    conn(:get, "/")
    |> SessionHelper.add_key
    |> assign(:current_user, current_user)
    |> send_resp(200, "")
    |> Channel.put_token(opts)
  end
  ...
  test "token is added for valid current user" do
    {:ok, user} = call(@current_user, []) |> verify_token
  ...
  test "token_data is configurable" do
    {:ok, user} = call(@current_user, token_data: &%{"user_id" => &1.username}) |> verify_token
    assert user == %{"user_id" => "Raymond"}
  end

  test "can capture a simple term (not a map) for the token_data" do
    {:ok, user} = call(@current_user, token_data: &(&1.username)) |> verify_token
    assert user == "Raymond"
  end

  test "raises when token_data is not a function" do
    assert_raise BadFunctionError, fn -> call(@current_user, token_data: "fred") end
  end

  test "log reports message when token added" do
    assert capture_log(fn ->
      call(@current_user, []) |> verify_token
