defmodule Phauxth.ConfirmTest do
  ...
  alias Phauxth.{Config, Confirm, TestAccounts}
  alias Phoenix.Token
  ...
  setup do
    ...
    valid_email = Token.sign(@endpoint, @user_salt, %{"email" => "fred+1@example.com"})
  ...
  test "confirmation fails for expired token" do
    expired_email = Token.sign(@endpoint, @user_salt, %{"email" => "ray@example.com"})
  ...
  test "key options passed on to the token module" do
    valid_email =
      Token.sign(@endpoint, @user_salt, %{"email" => "fred+1@example.com"}, key_iterations: 10)
