defmodule <%= base %>.Web.UserController do
  ...
  def create(conn, %{"user" => %{"email" => email} = user_params}) do
    {key, link} = Phauxth.Confirm.gen_token_link(email)<% else %>
  def create(conn, %{"user" => user_params<%= if confirm do %>, key<% end %>}) do<% end %><%= if api do %>
    with {:ok, %User{} = user} <- Accounts.create_user(user_params) do<%= if confirm do %>
      <%= base %>.Message.confirm_request("", link)<% end %>
      conn
      |> put_status(:created)
